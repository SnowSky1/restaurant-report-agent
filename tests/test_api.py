...
class ApiTests(unittest.TestCase):
    ...
    def test_analyze_serializes_complete_response(self):
        ...
        state = {
            ...
            "provenance": {"used_mock_data": True, "used_real_data": False, "warnings": ["测试回退"]},
            "errors": [],
        }
        with patch("api.main.run_analysis", new=AsyncMock(return_value=(report, state))):
    ...
    def test_analyze_rejects_report_without_location(self):
        with patch(
            ...
            response = self.client.post(
                "/api/analyze",
                json={"store_name": "测试店", "store_address": "无效地址", "use_llm": False},
            )
